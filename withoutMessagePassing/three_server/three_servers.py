...
RANDOM_SEED = 42
NEW_REQUEST = 1000000  # Total number of customers
...
def generator(_env, number, _s1_counter, _s2_counter, _s3_counter):
    ...
    for i in range(number):
        s1 = server_one(_env, i, _s1_counter, _s2_counter, _s3_counter)
        env.process(s1)
        arrival_ = random.expovariate(arrival)
        yield env.timeout(arrival_)


def server_one(_env, i, _s1_counter, _s2_counter, _s3_counter):
    ...
    with _s1_counter.request() as req:
        ...
        s2 = server_two(_env, i, _s2_counter, _s3_counter)
        env.process(s2)
...
def server_two(_env, i, _s2_counter, _s3_counter):
    ...
    with _s2_counter.request() as req:
        ...
        s3 = server_three(env, i, s3_counter)
        env.process(s3)
        ...
        s2_file.write(str(arrive) + "," + str(i) + "," + str(waiting_time) + "," + str(processing_time) + "," + str(
            len(_s2_counter.queue)) + "\n")


def server_three(_env, i, _s3_counter):
    ...
    with _s3_counter.request() as req:
        ...
        s3_file.write(str(arrive) + "," + str(i) + "," + str(waiting_time) + "," + str(processing_time) + "," + str(
            len(_s3_counter.queue)) + "\n")
...
def get_data():
    s1_dataset = pandas.read_csv("three_server/s1_" + str(arrival) + ".csv")
    s2_dataset = pandas.read_csv("three_server/s2_" + str(arrival) + ".csv")
    s3_dataset = pandas.read_csv("three_server/s3_" + str(arrival) + ".csv")
...
def get_plot():
    ...
    plt.plot(x_pdf3, y_pdf3, color='blue', label="Server 3:" + label)
    plt.legend()
    plt.xlabel('Latency')
    plt.ylabel('Probability')
    output_image = "three_server/AR_" + str(arrival) + ".png"
...
if __name__ == '__main__':
    ...
    s1_file = open("three_server/s1_" + str(arrival) + ".csv", "w")
    s1_file.write(columns + "\n")

    s2_file = open("three_server/s2_" + str(arrival) + ".csv", "w")
    s2_file.write(columns + "\n")

    s3_file = open("three_server/s3_" + str(arrival) + ".csv", "w")
    s2_file.write(columns + "\n")
